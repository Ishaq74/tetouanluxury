    ...
    <div className={`min-h-screen flex flex-col bg-stone-50 font-sans ${isRTL ? 'font-arabic' : ''}`}>
      ...
      <div className="bg-slate-900 text-slate-400 text-xs py-2 px-4 flex justify-between items-center relative z-[60]">
        ...
        <div className="flex items-center space-x-4 ml-auto">
          <span className="text-[10px] uppercase font-bold text-amber-500">Connexion Rapide :</span>
          <select 
            value={userRole} 
          ...
          >
            <option value="GUEST">Visiteur (Site Public)</option>
            <option value="CLIENT">Client (Espace Locataire)</option>
            <option disabled>──────────</option>
            <option value="STAFF_CLEANING">Employé (Fatima - App Mobile)</option>
            <option value="MANAGER">Manager (Sarah - Supervision)</option>
            <option disabled>──────────</option>
            <option value="ADMIN">Admin (Propriétaire - Global)</option>
          </select>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur-md text-white p-4 shadow-2xl z-[100] animate-slide-up border-t border-slate-700">
              <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
                  ...
                  <div className="flex space-x-4 shrink-0">
                      <button onClick={() => setShowCookieBanner(false)} className="text-xs text-slate-400 hover:text-white font-bold uppercase px-4 py-2">Fermer</button>
                  </div>
              </div>
          </div>
      )}
