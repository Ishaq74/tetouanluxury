...
const HeroSlideshow: React.FC<{ title: string, subtitle: string, onBook: () => void, ctaLabel: string }> = ({ title, subtitle, onBook, ctaLabel }) => {
    ...
    return (
        <div className="relative h-[100vh] bg-slate-900 overflow-hidden">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 z-10">
                ...
                <button onClick={onBook} className="bg-white/10 backdrop-blur-md border border-white/30 text-white px-10 py-4 text-sm font-bold uppercase tracking-widest hover:bg-white hover:text-slate-900 transition duration-500 transform hover:scale-105 animate-slide-up delay-200 shadow-2xl">{ctaLabel}</button>
            </div>
        </div>
    );
};
...
export const VillasPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-16">
      <SEO title={t('title_our_collection')} description={t('home_section_desc')} />
      ...
      <div className="text-center mb-10 pt-10">
        ...
        <div className="bg-white shadow-xl border border-stone-100 rounded-lg p-2 inline-flex flex-col md:flex-row items-center gap-4 max-w-4xl mx-auto mb-10 transform -translate-y-4">
            <div className="flex items-center px-4 py-3 border-b md:border-b-0 md:border-r border-stone-100 w-full md:w-64">
                ...
                <div className="text-left w-full">
                    <p className="text-[10px] font-bold uppercase text-slate-400">{t('step_dates')}</p>
                    <input type="text" placeholder={t('btn_add')} className="w-full text-sm font-bold text-slate-800 outline-none placeholder-slate-300"/>
                </div>
            </div>
            <div className="flex items-center px-4 py-3 w-full md:w-48">
                <User size={18} className="text-amber-600 mr-3" />
                <div className="text-left w-full">
                    <p className="text-[10px] font-bold uppercase text-slate-400">{t('label_guests')}</p>
                    <select 
                        value={guestCount} 
                        onChange={(e) => setGuestCount(Number(e.target.value))}
                        className="w-full text-sm font-bold text-slate-800 outline-none bg-transparent cursor-pointer"
                    >
                        {[1,2,3,4,5,6,7,8].map(n => <option key={n} value={n}>{n} {t('label_guests')}</option>)}
                    </select>
                </div>
            </div>
        ...
        <div className="flex justify-center space-x-2">
            <button onClick={() => setViewMode('LIST')} className={`flex items-center px-6 py-2 rounded-full text-xs font-bold uppercase transition ${viewMode === 'LIST' ? 'bg-slate-900 text-white shadow-md' : 'bg-stone-100 text-slate-500 hover:text-slate-900'}`}><List size={14} className="mr-2"/> {t('view_list')}</button>
            <button onClick={() => setViewMode('MAP')} className={`flex items-center px-6 py-2 rounded-full text-xs font-bold uppercase transition ${viewMode === 'MAP' ? 'bg-slate-900 text-white shadow-md' : 'bg-stone-100 text-slate-500 hover:text-slate-900'}`}><MapIcon size={14} className="mr-2"/> {t('view_map')}</button>
        </div>
      </div>
              ...
              <div key={villa.id} className="bg-white group hover:shadow-2xl transition duration-500 border border-stone-200 flex flex-col h-full">
                ...
                <div className="p-8 flex-1 flex flex-col">
                    ...
                    <div className="flex justify-between text-xs text-slate-500 mb-6 border-t border-stone-100 pt-4">
                        <span>{villa.bedrooms} {t('label_bedrooms')}</span>
                        <span>{villa.bathrooms} {t('label_bathrooms')}</span>
                        <span>{villa.pool ? t('label_pool') : ''}</span>
                    </div>
                          ...
                          >
                              <div className="relative">
                                  ...
                                  <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 bg-white px-3 py-2 rounded shadow-lg opacity-0 group-hover:opacity-100 transition whitespace-nowrap z-20 pointer-events-none">
                                      ...
                                      <p className="text-[10px] text-amber-600 font-bold">{villa.pricePerNight}€ {t('label_night')}</p>
...
export const VillaDetailsPage: React.FC = () => {
    const { villas } = useData();
    const { resolveContent, t } = useLanguage();
      ...
      <div className="animate-fade-in pb-20 relative">
         ...
         <div className="relative h-[70vh]">
            ...
            <div className="absolute bottom-0 left-0 p-8 md:p-16 text-white w-full">
                <Link to="/villas" className="flex items-center text-xs font-bold uppercase tracking-widest mb-4 hover:text-amber-400 transition"><ArrowLeft size={16} className="mr-2" /> {t('btn_back')}</Link>
         ...
         <div className="max-w-7xl mx-auto px-4 py-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
             <div className="lg:col-span-2 space-y-12">
                 <div>
                    <h2 className="font-serif text-2xl text-slate-900 mb-4">{t('nav_villas')}</h2>
                    <p className="text-slate-600 leading-relaxed text-lg whitespace-pre-line">{resolveContent(villa.description)}</p>
                 </div>
                 
                 <div>
                    <h2 className="font-serif text-2xl text-slate-900 mb-4">{t('feat_amenities')}</h2>
             ...
             <div className="lg:col-span-1">
                 <div className="sticky top-24 bg-white border border-stone-200 p-8 shadow-xl rounded-sm">
                     <div className="flex justify-between items-center mb-6"><span className="text-slate-500">{t('label_price')}</span><span className="text-3xl font-serif text-slate-900">{villa.pricePerNight}€</span></div>
                     <div className="space-y-4 mb-6">
                         <div className="flex justify-between text-sm text-slate-600 border-b border-stone-100 pb-2"><span>{t('label_bedrooms')}</span><span className="font-bold">{villa.bedrooms}</span></div>
                         <div className="flex justify-between text-sm text-slate-600 border-b border-stone-100 pb-2"><span>{t('label_bathrooms')}</span><span className="font-bold">{villa.bathrooms}</span></div>
                         <div className="flex justify-between text-sm text-slate-600 border-b border-stone-100 pb-2"><span>{t('label_pool')}</span><span className="font-bold">{villa.pool ? 'Yes' : 'No'}</span></div>
                     </div>
                     <Link to="/booking" search={{ villa: villa.id }} className="block w-full text-center py-4 bg-amber-600 text-white font-bold uppercase tracking-widest hover:bg-amber-700 transition">{t('cta_book_now')}</Link>
                 </div>
             </div>
         </div>
      </div>
...
export const BookingPage: React.FC = () => {
    ...
    // Basket State (using localStorage to persist during checkout)
    ...
    const [basket, setBasket] = useState<{villa: Villa, dates: {start: Date, end: Date}}[]>(() => {
        const saved = localStorage.getItem('booking_basket');
        if (saved) {
            try {
                // Revive dates
                const parsed = JSON.parse(saved);
                return parsed.map((i:any) => ({...i, dates: {start: new Date(i.dates.start), end: new Date(i.dates.end)}}));
            } catch (e) { return []; }
        }
        return [];
    });
    ...
    // Calculate totals using specific Engine logic
    const calculateTotals = () => {
        ...
        const itemsToCalculate = [
            ...basket, 
            ...(activeVilla && currentDateRange.start && currentDateRange.end ? [{ villa: activeVilla, dates: { start: currentDateRange.start, end: currentDateRange.end } }] : [])
        ];

        itemsToCalculate.forEach(item => {
            const pricing = calculateStayPrice(item.villa, item.dates.start, item.dates.end);
            subtotal += pricing.baseTotal;
            totalNights += pricing.nights;
            cleaningFee += pricing.cleaningFee;
            taxes += pricing.taxes;
            if (pricing.isLongStay) isLongStay = true;
        });

        // Global discount logic application (simplified for aggregate)
        let discount = 0;
        if (isLongStay) discount = subtotal * 0.10; // 10%

        const finalTotal = Math.max(0, subtotal - discount + cleaningFee + taxes - manualDiscount);

        return { subtotal, cleaningFee, taxes, finalTotal, isLongStay, discount };
    };
    ...
    const handleConfirmBooking = () => {
        ...
        localStorage.removeItem('booking_basket'); // Clear basket
        ...
        <div className="max-w-7xl mx-auto px-4 py-12 animate-fade-in pt-32">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="lg:col-span-2">
                        ...
                        <div className="space-y-12 animate-slide-up">
                            ...
                            <section>
                                ...
                                <div className="bg-amber-50 p-4 rounded border border-amber-200 mb-4 text-xs text-amber-900 flex items-start">
                                    ...
                                    <div>
                                        <p className="font-bold mb-1">{t('book_dynamic_title')}</p>
                                        <p>{t('book_dynamic_desc')}</p>
                                    </div>
                                </div>
                                <BookingCalendar 
                                    bookings={bookings} 
                            ...
                            {selectedVillaId && currentDateRange.start && currentDateRange.end && (
                                <div className="bg-stone-50 p-4 border border-stone-200 rounded flex justify-between items-center animate-fade-in">
                                    ...
                                    >
                                        <Plus size={14} className="inline mr-1"/> Ajouter au panier
                                    </button>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-y-8 animate-slide-up">
                            ...
                            <div className="bg-white p-8 rounded-sm shadow-sm border border-stone-200">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div><label className="label-xs">{t('label_firstname')}</label><input className="input-std" value={guestInfo.firstName} onChange={e => setGuestInfo({...guestInfo, firstName: e.target.value})} /></div>
                                    <div><label className="label-xs">{t('label_lastname')}</label><input className="input-std" value={guestInfo.lastName} onChange={e => setGuestInfo({...guestInfo, lastName: e.target.value})} /></div>
                                    <div><label className="label-xs">{t('label_email')}</label><input type="email" className="input-std" value={guestInfo.email} onChange={e => setGuestInfo({...guestInfo, email: e.target.value})} /></div>
                                    <div><label className="label-xs">{t('label_phone')}</label><input type="tel" className="input-std" value={guestInfo.phone} onChange={e => setGuestInfo({...guestInfo, phone: e.target.value})} /></div>
                                </div>
                                <div><label className="label-xs">{t('label_requests')}</label><textarea className="input-std h-32" placeholder={t('placeholder_requests')} value={guestInfo.requests} onChange={e => setGuestInfo({...guestInfo, requests: e.target.value})} /></div>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 3 && (
                        <div className="space-y-8 animate-slide-up">
                            <h2 className="font-serif text-2xl text-slate-900 mb-6 flex items-center"><CreditCard size={24} className="mr-3 text-amber-600"/> {t('title_payment')}</h2>
                            <div className="bg-white p-8 rounded-sm shadow-sm border border-stone-200 space-y-6">
                                {/* Promo */}
                                <div>
                                    <label className="label-xs">{t('book_promo_label')}</label>
                                    <div className="flex space-x-2"><input type="text" placeholder="Ex: WELCOME20" className="input-std flex-1" value={promoCode} onChange={(e) => setPromoCode(e.target.value)} /><button onClick={handleApplyPromo} className="bg-slate-900 text-white px-6 font-bold uppercase text-xs rounded-sm hover:bg-slate-800">{t('book_promo_btn')}</button></div>
                                </div>
                                {/* Methods */}
                                <div className="space-y-4">
                                    <label className={`flex items-center p-4 border rounded-sm cursor-pointer transition ${paymentMethod === 'CARD' ? 'border-amber-600 bg-amber-50/50' : 'border-stone-200 hover:bg-stone-50'}`}>
                                        <input type="radio" name="pay" className="mr-4" checked={paymentMethod === 'CARD'} onChange={() => setPaymentMethod('CARD')} />
                                        <div><span className="block font-bold text-slate-900">{t('book_method_card')}</span><span className="block text-xs text-slate-500">{t('book_method_card_desc')}</span></div>
                                    </label>
                                    <label className={`flex items-center p-4 border rounded-sm cursor-pointer transition ${paymentMethod === 'TRANSFER' ? 'border-amber-600 bg-amber-50/50' : 'border-stone-200 hover:bg-stone-50'}`}>
                                        <input type="radio" name="pay" className="mr-4" checked={paymentMethod === 'TRANSFER'} onChange={() => setPaymentMethod('TRANSFER')} />
                                        <div><span className="block font-bold text-slate-900">{t('book_method_transfer')}</span><span className="block text-xs text-slate-500">{t('book_method_transfer_desc')}</span></div>
                                    </label>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="flex justify-between mt-12 pt-8 border-t border-stone-200">
                        {step > 1 ? <button onClick={() => setStep(step - 1)} className="px-8 py-4 text-slate-500 font-bold uppercase tracking-widest text-xs hover:text-slate-900 transition">{t('btn_back')}</button> : <div></div>}
                        <button onClick={step === 3 ? handleConfirmBooking : handleNext} disabled={step === 1 && basket.length === 0 && (!currentDateRange.start || !currentDateRange.end)} className="px-10 py-4 bg-slate-900 text-white font-bold uppercase tracking-widest text-xs shadow-xl hover:bg-slate-800 hover:shadow-2xl transition transform hover:-translate-y-1">{step === 3 ? t('btn_confirm') : t('btn_next')}</button>
                    </div>
                </div>
                        ...
                        {basket.map((item, index) => (
                            <div key={index} className="mb-6 pb-6 border-b border-stone-100 relative group">
                                ...
                                <div className="flex items-start space-x-4">
                                    <div className="w-16 h-16 rounded-sm overflow-hidden shrink-0"><img src={item.villa.images[0]} className="w-full h-full object-cover" /></div>
                                    <div><p className="font-bold text-slate-900 leading-tight mb-1">{item.villa.name}</p><p className="text-xs text-slate-600">{item.dates.start.toLocaleDateString()} - {item.dates.end.toLocaleDateString()}</p></div>
                                </div>
                            </div>
                        ))}
                        {activeVilla && currentDateRange.start && currentDateRange.end && (
                            <div className="mb-6 pb-6 border-b border-stone-100 relative opacity-50">
                                <p className="text-xs uppercase font-bold text-amber-600 mb-2">Sélection en cours</p>
                                <div className="flex items-start space-x-4">
                                    <div className="w-16 h-16 rounded-sm overflow-hidden shrink-0"><img src={activeVilla.images[0]} className="w-full h-full object-cover" /></div>
                                    <div><p className="font-bold text-slate-900 leading-tight mb-1">{activeVilla.name}</p><p className="text-xs text-slate-600">{currentDateRange.start.toLocaleDateString()} - {currentDateRange.end.toLocaleDateString()}</p></div>
                                </div>
                            </div>
                        )}
                        {(totals.subtotal > 0) ? (
                            <div className="space-y-3 text-sm pt-2">
                                <div className="flex justify-between text-slate-600"><span>{t('book_summary_accom')}</span><span>{totals.subtotal.toFixed(0)}€</span></div>
                                {totals.isLongStay && <div className="flex justify-between text-green-600 text-xs"><span>{t('book_summary_discount')}</span><span>-{totals.discount.toFixed(0)}€</span></div>}
                                <div className="flex justify-between text-slate-600"><span>{t('book_summary_clean')}</span><span>{totals.cleaningFee}€</span></div>
                                <div className="flex justify-between text-slate-600"><span>{t('book_summary_tax')}</span><span>{totals.taxes.toFixed(0)}€</span></div>
                                {manualDiscount > 0 && <div className="flex justify-between text-green-600 font-bold"><span>Promo</span><span>-{manualDiscount.toFixed(0)}€</span></div>}
                                <div className="flex justify-between items-end pt-4 border-t border-stone-200"><span className="font-bold text-lg text-slate-900">{t('total_amount')}</span><span className="font-serif text-2xl text-amber-600">{totals.finalTotal.toFixed(0)}€</span></div>
                            </div>
                        ) : <div className="text-center py-8 text-slate-400"><p className="text-sm">Panier vide</p></div>}
                    </div>
                </div>
            </div>
        </div>
