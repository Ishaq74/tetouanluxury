...
import { MaintenanceTicket, StaffMember, UserRole, InventoryItem } from '../../../types';
...
export const MaintenanceManager = () => {
    const { tickets, addTicket, updateTicketStatus, deleteTicket } = useData();
...
export const StaffManager = () => {
    const { showToast } = useToast();
    const { staff, updateStaffSchedule, addStaffMember, updateStaffMember, deleteStaffMember } = useData(); 
...
export const InventoryManager = () => {
    const { inventory, updateInventory, addInventoryItem, updateInventoryItem, deleteInventoryItem } = useData(); 
    const { showToast } = useToast();
...
// --- 4. CONCIERGE MANAGER ---
export const ConciergeManager = () => {
    const { showToast } = useToast();
    const { serviceRequests, updateServiceRequestStatus } = useData(); 
