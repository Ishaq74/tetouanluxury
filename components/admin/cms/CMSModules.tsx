...
const IndexPageEditor = ({ type }: { type: string }) => {
    const { indexPageSettings, updateIndexPageSettings } = useData();
...
const CategoryManager = ({ type }: { type: 'BLOG' | 'GUIDE' | 'SERVICE' }) => {
...
// --- 1. BLOG MANAGER ---
export const BlogManager = ({ subTab, setSubTab }: { subTab?: string; setSubTab?: (val: string) => void }) => {
    const { showToast } = useToast();
    const { blogPosts, addBlogPost, updateBlogPost, deleteBlogPost, categories } = useData();
    ...
    return (
        <div className="h-full flex flex-col animate-fade-in relative bg-stone-50/50">
            ...
            {subTab === 'CATEGORIES' && <CategoryManager type="BLOG" />}
            {subTab === 'INDEX_SETTINGS' && <IndexPageEditor type="BLOG" />}
...
// --- 2. GUIDE MANAGER ---
export const GuideManager = ({ subTab, setSubTab }: { subTab?: string; setSubTab?: (val: string) => void }) => {
    const { showToast } = useToast();
    const { guideItems, addGuideItem, updateGuideItem, deleteGuideItem, categories } = useData();
    const [isEditorOpen, setIsEditorOpen] = useState(false);
    const [editorTab, setEditorTab] = useState('DETAILS');
    ...
    return (
        <div className="h-full flex flex-col animate-fade-in relative bg-stone-50/50">
            ...
            {subTab === 'CATEGORIES' && <CategoryManager type="GUIDE" />}
            {subTab === 'INDEX_SETTINGS' && <IndexPageEditor type="GUIDE" />}
...
export const ServiceManager = ({ subTab, setSubTab }: { subTab?: string; setSubTab?: (val: string) => void }) => {
    const { showToast } = useToast();
    const { premiumServices, addPremiumService, updatePremiumService, deletePremiumService } = useData();
    const [isEditorOpen, setIsEditorOpen] = useState(false);
    const [editorTab, setEditorTab] = useState('DETAILS');
    ...
    return (
        <div className="h-full flex flex-col animate-fade-in relative bg-stone-50/50">
            ...
            {subTab === 'INDEX_SETTINGS' && <IndexPageEditor type="SERVICES" />}
...
export const TranslationManager = () => {
    const { translations, updateTranslation } = useData();
    const [search, setSearch] = useState('');
